...
| Integration tests | PHPUnit inside wp-env | Behaviour against a real WordPress + MySQL: the log tables, one row per recipient, interception, suppression, privacy export/erase, purge. | `tests-integration.yml` | `make test-integration` |
...
**Integration** (`tests/Integration/`) runs inside the wp-env *tests* container against a real WordPress and database. No SMTP server: sends are short-circuited on `pre_wp_mail` the way an API-based mail plugin would, which exercises that path too. Every test starts with all plugin options deleted — site and network, the list of providers included — so nothing leaks between tests. It skips itself, with the command that fixes it, when the site is still the network the multisite suite left behind.

**End-to-end** (`tests/e2e/run.sh`) is the only suite that walks the whole chain — config → `phpmailer_init` → SMTP → mailbox. It starts Mailpit on the wp-env Docker network and writes a provider into the plugin's list pointing at it — a record, not the flat options, which still answer only for an install whose migration has not run and would leave the suite testing a path almost nobody is on. Then, in order: sends a real `wp_mail()` with a Cc and checks through Mailpit's HTTP API that it arrived with the right From, To, Cc and body; checks the log has one row per recipient in `sent` and that the message's `Message-ID` header is the log's id; checks the SMTP transcript was stored and that no body was; checks a user's profile finds their mail; runs `wp diluxone-mail test`; breaks the port on purpose and checks the `failed` row carries the real SMTP error; adds a second provider underneath, breaks the first, and checks the message goes out through the second — exactly once in the mailbox, twice in the log, each row naming the provider that carried it; checks `status`; sends with a Bcc; drops in an mu-plugin on `phpmailer_init` and checks it does *not* demote the plugin to observer and that the sender stays ours; drops in one that answers `pre_wp_mail` and checks that one does; drops in a fake `pre_wp_mail` interceptor with a closure and checks the log names it, then turns on the detach option and checks mail flows again; exports and erases a person's data; runs the purge; and diagnoses a live domain. It found a real bug on its first run — the From override was applied too late for sites on `localhost` — that no other suite could have seen.
...
One thing worth knowing about both container suites: WordPress on a network looks the current site up by `HTTP_HOST`, and when it does not recognise the host it redirects and exits inside its own load — with no body, because there is nobody to send one to. PHPUnit then prints nothing and exits 0, and every runner above it reports success for a suite that never ran a test. The multisite suite did exactly that until it was found. The bootstrap now reads the network's own host out of `wp-config.php` before loading WordPress, and both runners read the output and fail when there is no result line in it rather than trusting the exit code.
