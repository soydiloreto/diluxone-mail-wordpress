...
For maintainers. End users get the plugin from [wordpress.org/plugins/diluxone-mail](https://wordpress.org/plugins/diluxone-mail/) — they don't need to read this document.
...
The PHP `Version:` header in `diluxone-mail.php` and the `DILUXONE_MAIL_VERSION` constant carry a `-dev` suffix on `main` between releases. The `Stable tag:` in `readme.txt` does **not** — it always holds the last published release, or before any release the next intended one.

| State | PHP `Version:` | `DILUXONE_MAIL_VERSION` | readme `Stable tag:` |
| --- | --- | --- | --- |
...
8. **Bump back to dev.** Open a follow-up PR `chore/bump-(X.Y.Z+1)-dev` that:
   - Sets PHP `Version:` and `DILUXONE_MAIL_VERSION` to `(next intended version)-dev`.
